...
async fn main() ->  Result<(), Box<dyn std::error::Error>>{
    ...
    if enable_fib.eq("true") {
        ...
        let fib_of_extracted_numbers: Vec<BigInt> = extracted_numbers
            ...
            .collect();
        println!("fib of found numbers are: {:?}", fib_of_extracted_numbers);

        let comment_body = format!("{:?}", fib_of_extracted_numbers);
        manage_pr::PullRequest::post_comment_to_pr( github_repository.as_str(), comment_body.as_str(), pr_number).await?;
    } else {
